...
rocket-guard-rs is the reserved namespace for the Rocket adapter of the Guard ecosystem. It will wire the [guard-core-rs](https://github.com/rennf93/guard-core-rs) detection engine into Rocket as application-layer security middleware (a Rocket Fairing).
...
│   ├── rocket-guard-rs  <- This repo: Rocket Fairing adapter (scaffold)
│   ├── tower-guard-rs   <- Sibling adapter (scaffold; generic tower::Layer/Service)
...
1. **Dependencies**: `guard-core-rs` (facade crate, re-exports `compiler`, `preprocessor`, `semantic`) plus `rocket`. Versions to be chosen when implementation starts.
2. **Fairing**: implement Rocket's `Fairing` trait, attaching an `on_request` callback that inspects incoming requests before routing/handlers run. A Fairing is the idiomatic Rocket extension point for cross-cutting request checks.
3. **Per request**: extract method, path, headers, client IP, and body; call the CPU-bound engine functions synchronously (the engine has no I/O and no tokio dependency, so it can run inside Rocket's async context without spawning); deny with a 403 response when the engine returns a threat verdict.
...
- **Dependencies**: none today. Planned: `guard-core-rs` and `rocket`.
